...
def send_data(value1, value2):
    data = f"{value1} {value2}\n"  # Format the data as a string with a newline at the end
    ser.write(data.encode())  # Send the encoded string to the Arduino
    print(f"Sent: {data}")
    # Wait to make sure the Arduino has processed the data and sent a response
    time.sleep(0.1)  # Short delay to allow Arduino to process and respond
    if ser.in_waiting > 0:
...
def record_video(output_filename='output.mp4', fps=20.0, resolution=(640, 480)):
    global set_time
    now = time.time()
    cap = cv2.VideoCapture(0)#setup 0 là laptop, 1 là camera thiết bị ngoài
    ...
    while True:
        ...
        print(x_target, y_target, "----" ,int(height * 0.98), int(width / 2))
        send_data(y_target , int(width/2))
        time.sleep(0.1)
        #banhxe1 > 320 > banhxe2
        out.write(edges_img)
